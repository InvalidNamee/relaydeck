...
列表和认证状态；页面按工作分组分配控制权，其他客户端可以实时观看或申请控制整个分组。
...
- 远端画面聚焦时可使用 `Ctrl/Cmd+C` 和 `Ctrl/Cmd+V` 进行双向纯文本剪贴板操作。
...
4. 两台设备分别控制不同分组时，可以同时独立操作；申请已占用分组时，必须由当前控制者同意。
5. 标签页右侧的 `×` 可以关闭该页；删除分组不会关闭页面，剩余页面会迁入默认工作区。
6. 侧栏显示当前在线设备及其控制的分组；控制者释放或断开后，最早等待的申请者自动取得控制权。

客户端“设置”中可以为所有网关指定统一的新页面默认地址；留空时打开空白页。客户端设置的
地址优先于网关的 `DEFAULT_URL`，并只保存在当前客户端。
...
| `DEFAULT_URL` | `about:blank` | 客户端未指定地址时的新页面默认地址 |
...
- 同一分组不支持两个设备同时写入，采用控制者审批的移交流程以避免输入互相干扰。
